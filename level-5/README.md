...
Once the server is running, you can interact with it by sending JSON requests to its standard input, specifying the tool and parameters. The tools now accept either a file path (`pdf_path`) or a base64 encoded string (`pdf_base64`) of the PDF content. The server will respond with a JSON output to standard output.

Example request (conceptual - using pdf_base64):
...
Example request (conceptual - using pdf_path):
